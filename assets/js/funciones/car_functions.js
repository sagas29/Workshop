$(document).ready(function () {
    ...
    $('#editable').DataTable({
        ...
        "order": [[0, "asc"]],
        "ajax": {
            url: base_url+'Vehiculos/get_data',
            type: 'POST',
    ...
    $("#brand").change(function()
    {
        ...
        $.ajax({
            url:base_url+"Vehiculos/get_models",
            type: "POST",
            data: ajaxdata,
...
$(document).on("click",".state_car", function(event)
{
    let id_car = $(this).attr("id");
    let data = $(this).attr("data-state");
    state_client(id_car,data);
});
...
function deleted(id_car) {
    ...
    Swal.fire({
        ...
        if (result.value) {
            $.ajax({
                type: "POST",
                url: base_url+"Vehiculos/eliminar",
                data: dataString,
                dataType: 'json',
...
function state_client(id_car, data) {
    let token =   $("#get_csrf_hash").val()
    let dataString = "id=" + id_car + "&csrf_token_name="+token;
    Swal.fire({
        title: 'Alerta!!',
        text: "Estas seguro de "+ data+" este cliente?!",
        type: 'warning',
        showCancelButton: true,
        confirmButtonColor: '#3085d6',
        cancelButtonColor: '#d33',
        ...
        if (result.value) {
            $.ajax({
                type: "POST",
                url: base_url+"Vehiculos/state_car",
                data: dataString,
                dataType: 'json',
                success: function (data) {
                    notification(data.type,data.title,data.msg);
...
function save_data(){
    ...
    $.ajax({
        type: 'POST',
        url: base_url+'Vehiculos/agregar',
        cache: false,
